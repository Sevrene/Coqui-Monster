  ...
  return function onlyWhenParentIsValidator<RuleType>(
    ...
    let shouldValidate = Array.isArray(parentValue)
      ? parentValue.includes(
          (context.parent as Record<string, any>)?.[parentName]
        )
...
export function validPostTimestamps(
  ...
  return function validPostTimestampsValidator<RuleType>(
    ...
    const compare =
      order === 'before' ? thatField > thisField : thatField < thisField;
 ...
 */
export function mustStartWith(
  startWith: string | Array<string>,
  message?: string
) {
...
export function linkMustBeRelativeOrAbsolute(message?: string) {
  return function linkMustBeRelativeOrAbsoluteValidator<RuleType>(
    value: RuleType
  ) {
    if (typeof value === 'string') {
...
export function isSlug(message?: string) {
  return matchesRegex(
    stringIsSlug,
    message ||
      'Must contain only lowercase letters [a-z], numbers [0-9], and dashes ("-"). Must start and end with a letter or number. Must not contain consecutive special characters.'
  );
}
